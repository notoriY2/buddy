...
<head>
    ...
    function showDepartment(str) {
        if (str == "") {
            ...
            hideCourseDropdown(); // Ensure the course dropdown remains hidden
            return;
        } else {
            ...
            xmlhttp.onreadystatechange = function() {
                if (this.readyState == 4 && this.status == 200) {
                    ...
                    // Add onchange event listener to department dropdown (if any)
                    var departmentDropdown = document.querySelector('#txtHint select[name="department_id"]');
    ...
    document.addEventListener('DOMContentLoaded', function() {
        hideCourseDropdown(); // Hide the course dropdown initially
    });
</script>
</head>
<body>
                                ...
                                <form method="POST" action="">
                                            ...
                                            <div class="form-group">
                                                ...
                                                <?php 
                                                    ...
                                                    if (mysqli_num_rows($query) > 0) {                       
                                                        ...
                                                        while ($row = mysqli_fetch_array($query)) {
                                                            echo '<option value="' . htmlspecialchars($row['session_id']) . '">' . htmlspecialchars($row['sessionName']) . '</option>';
                                                        }
                                                        echo '</select>';
                                                    }
                                                ?>   
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-6">
                                            <div class="form-group">
                                                <div id="txtHint">
                                                    <!-- Department options will be loaded here by AJAX -->
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <label for="course_id" class="control-label mb-1">Course</label>
                                    <?php 
                            ...
                            <tbody>
                                <?php
                                    if (isset($_POST['submit'])) {
                                        ...
                                        $query = "
                                            SELECT DISTINCT student.student_id, student.firstName, student.lastName, student.studentNo,
                                                student.dateCreated, level.levelName, faculty.facultyName, department.departmentName, course.courseTitle, 
          course.semester_id, student.level_id, student.session_id, student.faculty_id, student.department_id
                                            FROM student
                                            ...
                                            WHERE student.level_id = '$level_id'
                                                AND student.session_id = '$session_id'
                                                AND student.department_id = '$department_id'
                                                AND student.faculty_id = '$faculty_id'
                                                AND course_assignment.course_id = '$course_id'
                                        ";
                                ...
                                <tr>
                                    ...
                                    <td>
    <a href="computeSemesterResults.php?
        semester_id=1&
        studentNo=<?php echo urlencode($row['studentNo'] ?? ''); ?>&
        level_id=<?php echo urlencode($row['level_id'] ?? ''); ?>&
        faculty_id=<?php echo urlencode($row['faculty_id'] ?? ''); ?>&
        department_id=<?php echo urlencode($row['department_id'] ?? ''); ?>&
        session_id=<?php echo urlencode($row['session_id'] ?? ''); ?>&
        course_id=<?php echo urlencode($row['course_id'] ?? ''); ?>&
        semester=first" 
        title="View First Semester Courses">
...
<td>
    <a href="computeSemesterResults.php?
        ...
        course_id=<?php echo urlencode($row['course_id'] ?? ''); ?>&
        semester=second" 
        title="View Second Semester Courses">
        <i class="fa fa-eye fa-1x"></i> View Course
    </a>
</td>
