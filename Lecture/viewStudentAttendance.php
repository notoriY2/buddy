...
if (isset($_POST['view'])) {
    ...
    $studentNo = $_POST['student'];
    $type = $_POST['type'];
    $dateTaken = $_POST['dateTaken'] ?? null;
    $toDate = $_POST['toDate'] ?? null; // For date range

    // Check if course, level, student number, and type are not empty
    if (!empty($course_id) && !empty($level_id) && !empty($studentNo) && !empty($type)) {
        $query = "SELECT 
                  ...
                  WHERE attendance.studentNo = ? AND attendance.course_id = ? AND attendance.level_id = ?";

        // Append conditions based on the selected type
        ...
        if (empty($statusMsg)) {
            ...
            // Bind parameters based on the type
            if ($type == '2') {
                // Type 2: By Single Date
                $stmt->bind_param("ssss", $studentNo, $course_id, $level_id, $dateTaken);
            } elseif ($type == '3') {
                // Type 3: By Date Range
                $stmt->bind_param("sssss", $studentNo, $course_id, $level_id, $dateTaken, $toDate);
            } else {
                // Type 1: All
                $stmt->bind_param("sss", $studentNo, $course_id, $level_id);
            }
            $stmt->execute();
            $results = $stmt->get_result();
        }
    } else {
        $statusMsg = "<div class='alert alert-danger' role='alert'>Please select all required fields!</div>";
    }
}
...
function typeDropDown(value) {
    ...
    var dateFieldContainer = document.getElementById('txtHinttttt');
    
    if (value == '2') { // Single Date
        dateRangeFields.style.display = 'none'; // Hide date range fields
        dateFieldContainer.innerHTML = '<label class="form-control-label">Date<span class="text-danger ml-2">*</span></label><input type="date" class="form-control" name="dateTaken" required>';
    } else if (value == '3') { // Date Range
        dateRangeFields.style.display = 'block'; // Show date range fields
        dateFieldContainer.innerHTML = '<label class="form-control-label">Start Date<span class="text-danger ml-2">*</span></label><input type="date" class="form-control" name="dateTaken" required><label class="form-control-label">End Date<span class="text-danger ml-2">*</span></label><input type="date" class="form-control" name="toDate" required>';
    } else { // Type is "All"
        dateRangeFields.style.display = 'none'; // Hide date fields
    ...
    <div class="animated fadeIn">
        <div class="row">
            <div class="col-lg-12">
                <div class="card mb-4">
                    <div class="card-header">
                        <strong class="card-title"><h2 align="center">View Student Attendance</h2></strong>
                        <?php echo $statusMsg; ?>
                    </div>
                    <div class="card-body">
                        <form method="post">
                            <div class="row">
                                ...
                                <div class="col-6">
                                    <div class="form-group">
                                        <label for="level_id" class="control-label mb-1">Select Level<span class="text-danger ml-2">*</span></label>
                                        <?php
                            ...
                            <div class="row">
                                <div class="col-6">
                                    <div class="form-group">
                                        <label for="student" class="control-label mb-1">Select Student<span class="text-danger ml-2">*</span></label>
                                        <input type="text" name="student" class="form-control" value="" required placeholder="Student Number">
                                    </div>
                                </div>
                                <div class="col-6">
            ...
            <div class="col-md-12">
                <div class="card mb-4">
                    <div class="card-header">
                        <strong class="card-title"><h2 align="center">Attendance Records</h2></strong>
                    </div>
                    <div class="card-body">
                    <table id="bootstrap-data-table" class="table table-hover table-striped table-bordered">
                            ...
                            <tbody>
                                <?php
                                // Display fetched attendance records
                                if (!empty($results)) {
