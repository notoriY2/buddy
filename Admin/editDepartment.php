...
if (isset($_GET['edit_id'])) {
    ...
    $stmt = $conn->prepare("SELECT * FROM department WHERE department_id=?"); // Use the correct column name
    $stmt->bind_param("i", $_SESSION['edit_id']);
    $stmt->execute();
...
if (isset($_POST['submit'])) {
    ...
    $faculty_check = $conn->prepare("SELECT COUNT(*) FROM faculty WHERE faculty_id = ?");
    $faculty_check->bind_param("i", $faculty_id);
    $faculty_check->execute();
    $faculty_check->bind_result($count);
    $faculty_check->fetch();
    $faculty_check->close();

    if ($count == 0) {
        $alertStyle = "alert alert-danger";
        $statusMsg = "The selected faculty ID does not exist!";
    } else {
        // Use the correct column name for update
        $stmt = $conn->prepare("UPDATE department SET departmentName=?, faculty_id=? WHERE department_id=?"); // Corrected column names
        $stmt->bind_param("sii", $departmentName, $faculty_id, $_SESSION['edit_id']);
        $query = $stmt->execute();
                                ...
                                <form method="post" action="">
                                    <div class="row">
                                            ...
                                            <div class="form-group">
                                                ...
                                                if ($count > 0) {                       
                                                    ...
                                                    while ($row = mysqli_fetch_array($query)) {
                                                        echo '<option value="'.htmlspecialchars($row['faculty_id']).'"'.($row['faculty_id'] == $rowi['faculty_id'] ? ' selected' : '').'>'.htmlspecialchars($row['facultyName']).'</option>';
                                                    }
                                                    echo '</select>';
                                                }
                                                ?>        
                                        ...
                                        <div class="col-6">
                                            ...
                                            <input id="departmentName" name="departmentName" type="text" class="form-control" value="<?php echo htmlspecialchars($rowi['departmentName']);?>" required>
                                        </div>
                                    </div>
                                    <div>
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        <table id="bootstrap-data-table" class="table table-hover table-striped table-bordered">
                            ...
                            <tbody>
                                <?php
                                $ret = mysqli_query($conn, "SELECT department.department_id AS department_id, department.departmentName, department.dateCreated, faculty.facultyName FROM department INNER JOIN faculty ON faculty.faculty_id = department.faculty_id");
                                $cnt = 1;
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <a href="editDepartment.php?edit_id=<?php echo htmlspecialchars($row['department_id']); ?>" title="Edit Department"><i class="fa fa-edit fa-1x"></i></a>
                                        <a onclick="return confirm('Are you sure you want to delete?')" href="../php/deleteDepartment.php?del_id=<?php echo htmlspecialchars($row['department_id']); ?>" title="Delete Department"><i class="fa fa-trash fa-1x"></i></a>
                                    </td>
                                </tr>
                                <?php 
                                    $cnt++;
