...
if (isset($_GET['editCourse_id'])) {
    $editCourse_id = htmlspecialchars($_GET['editCourse_id']);

    // Retrieve course details using a prepared statement
    $stmt = $conn->prepare("SELECT * FROM course WHERE courseCode = ?");
    $stmt->bind_param("s", $editCourse_id);
    $stmt->execute();
...
if (isset($_POST['submit'])) {
    ...
    // Update course details using a prepared statement
    $stmt = $conn->prepare("UPDATE course SET courseTitle = ?, courseCode = ?, courseUnit = ?, level_id = ?, semester_id = ?, pre_courseCode1 = ?, pre_courseCode2 = ? WHERE courseCode = ?");
    $stmt->bind_param("ssisisss", $courseTitle, $courseCode, $courseUnit, $level_id, $semester_id, $pre_courseCode1, $pre_courseCode2, $editCourse_id);

    if ($stmt->execute()) {
        // Remove existing course assignments
        $stmt = $conn->prepare("DELETE FROM course_assignment WHERE course_id = (SELECT course_id FROM course WHERE courseCode = ?)");
        $stmt->bind_param("s", $editCourse_id);
        $stmt->execute();

        // Insert new course assignment
        $query = $conn->prepare("INSERT INTO course_assignment (course_id, faculty_id, department_id) 
                                 SELECT course_id, ?, ? FROM course WHERE courseCode = ?");
        $query->bind_param("iis", $faculty_id, $department_id, $editCourse_id);

        if ($query->execute()) {
            $_SESSION['alertStyle'] = "alert alert-success";
            $_SESSION['statusMsg'] = "Course Updated Successfully!";
        } else {
            $_SESSION['alertStyle'] = "alert alert-danger";
            $_SESSION['statusMsg'] = "An error occurred during course assignment: " . $conn->error;
        }
    } else {
        $_SESSION['alertStyle'] = "alert alert-danger";
        $_SESSION['statusMsg'] = "An error occurred: " . $stmt->error;
    }
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title"><h2 align="center">Update Course</h2></strong>
                    </div>
                    <div class="card-body">
                        <div id="pay-invoice">
                            <div class="card-body">
                                ...
                                <div class="row">
                                        <div class="col-6">
                                            <div class="form-group">
                                                ...
                                                <input id="courseTitle" name="courseTitle" type="text" class="form-control" value="<?php echo htmlspecialchars($rowi['courseTitle']); ?>" required placeholder="Course Title">
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <label for="courseCode" class="control-label mb-1">Course Code</label>
                                            <input id="courseCode" name="courseCode" type="text" class="form-control" value="<?php echo htmlspecialchars($rowi['courseCode']); ?>" required placeholder="Course Code">
                                        </div>
                                    </div>
                                    <div class="row">
                                        <div class="col-6">
                                            <div class="form-group">
                                                ...
                                                <input id="courseUnit" name="courseUnit" type="text" class="form-control" value="<?php echo htmlspecialchars($rowi['courseUnit']); ?>" required placeholder="Course Unit">
                                            </div>
                                        </div>
                                        <div class="col-6">
                                            <div class="form-group">
                                                ...
                                                if ($count > 0) {                       
                                                    ...
                                                    while ($row = mysqli_fetch_array($query)) {
                                                        $selected = $row['level_id'] == $rowi['level_id'] ? 'selected' : '';
                                                        echo '<option value="'.$row['level_id'].'" '.$selected.'>'.$row['levelName'].'</option>';
                                                    }
                                                    echo '</select>';
                                                }
                                                ?>   
                                        ...
                                        <div class="col-6">
                                            <div class="form-group">
                                                ...
                                                if ($count > 0) {                       
                                                    ...
                                                    while ($row = mysqli_fetch_array($query)) {
                                                        $selected = $row['semester_id'] == $rowi['semester_id'] ? 'selected' : '';
                                                        echo '<option value="'.$row['semester_id'].'" '.$selected.'>'.$row['semesterName'].'</option>';
                                                    }
                                                    echo '</select>';
                                                }
                                                ?>                                                       
