...
if (isset($_GET['del_id']) && is_numeric($_GET['del_id'])) {
    ...
    if ($stmt->execute()) {
        ...
        $_SESSION['statusMsg'] = 'Department deleted successfully!';
    } else {
        $_SESSION['alertStyle'] = 'alert alert-danger';
        $_SESSION['statusMsg'] = 'An error occurred while deleting the department.';
    }

    $stmt->close();
} else {
    $_SESSION['alertStyle'] = 'alert alert-danger';
    $_SESSION['statusMsg'] = 'Invalid department ID.';
}


// Redirect to createFaculty.php
echo "<script type='text/javascript'>
    window.location = '../Admin/createDepartment.php';
    </script>";
?>
