...
<body>
        ...
        <div class="row">
            ...
            <div class="col-lg-12">
                <div class="card">
                    <div class="card-header">
                        <strong class="card-title">
                            <h2 align="center">Attendance Trend</h2>
                        </strong>
                    </div>
                    <div class="card-body">
    <!-- Attendance Trend Line Chart -->
    <div id="attendance-trend-chart-container">
    ...
    const dailyData = attendanceData.days.map((count, index) => {
        const days = attendanceData.totalDays;
        const attendancePercentage = (count / (days - index)) * 100;
        return attendancePercentage.toFixed(2);
    });

    // Initialize chart
    const attendanceTrendChart = new Chart(ctxLine, {
        type: 'line',
        data: {
            labels: attendanceData.totalDays <= 7 ? Array.from({ length: attendanceData.totalDays }, (_, i) => `Day ${i + 1}`) : attendanceData.weekLabels,
            datasets: [{
                label: 'Attendance Percentage',
                data: attendanceData.totalDays <= 7 ? dailyData : attendanceData.weeks,
