...
if ($pageId) {
    $stmtPage = $conn->prepare("SELECT page_id, page_title, page_description, image, student_id FROM `page` WHERE page_id = ?");
    $stmtPage->bind_param("i", $pageId);
    $stmtPage->execute();
    $resultPage = $stmtPage->get_result();
    
    if ($resultPage->num_rows > 0) {
        $pageRow = $resultPage->fetch_assoc();
        $pageName = htmlspecialchars($pageRow['page_title']);
        $pageDescription = htmlspecialchars($pageRow['page_description']);
        $pageImage = htmlspecialchars($pageRow['image']);
        $pageOwnerId = $pageRow['student_id'];
    } else {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['text_content'])) {
        ...
        $stmt = $conn->prepare("INSERT INTO post (student_id, page_id, text_content, image) VALUES (?, ?, ?, ?)");
        $stmt->bind_param("iiss", $studentId, $pageId, $textContent, $image);
        if ($stmt->execute()) {
        ...
        if ($action === 'follow') {
            ...
            if ($result->num_rows === 0) {
                ...
                if ($insertStmt->execute()) {
                    // Redirect to the previous page with a status parameter
header('Location: ' . $_SERVER['HTTP_REFERER'] . '?status=unfollowed');
exit;
                } else {
...
// Fetch posts with images for the page
$stmtPosts = $conn->prepare("SELECT text_content, image FROM post WHERE page_id = ? AND image != '' ORDER BY created_at DESC");
$stmtPosts->bind_param("i", $pageId);
$stmtPosts->execute();
...
.group-header {
    ...
    .photo-box {
    flex-basis: calc(33.333% - 10px); /* Keep the box one-third of the container width */
...
<div class="container">
    ...
    <div class="profile-container"  style="margin-top:60px">
        <img src="../images/61i.png" class="cover-img" alt="Cover Image">
        <div class="profile-details">
            <div class="pd-left">
                <div class="pd-row">
                <img src="../php/images/<?php echo $pageImage; ?>" alt="<?php echo $pageName; ?>" class="page-image">
                    <div>
        ...
        <div class="profile-info">
            <div class="info-col">
                <div class="profile-intro">
                    <h3>Intro</h3>
                    <p class="intro-text"><?php echo $pageDescription; ?></p>
                    <hr>
                    <ul>
                        <li><i class="fa-solid fa-circle-exclamation"></i> Page , App</li>
                        <li><i class="fa-solid fa-globe"></i> https/:Buddy</li>
                ...
                <?php if ($post['image']): ?>
                    <div>
                    <img src="../php/images/<?php echo htmlspecialchars($post['image']); ?>" alt="Post Image" class="post-image">
                    </div>
                <?php endif; ?>
            <?php endwhile; ?>
                    </div>
...
// Modify the query to fetch posts and check if they are shared
$stmt = $conn->prepare("
...
");
$stmt->bind_param("ii", $studentId, $pageId);
$stmt->execute();
$result = $stmt->get_result();
        ...
        <div class="feed">
            <div class="feed-top">
                                <div class="user">
                                    ...
                                    <img src="../php/images/<?php echo $pageImage; ?>" alt="<?php echo $pageName; ?>">
                                    </div>
                                    <div class="profile-pic" id="my-profile-picture">
