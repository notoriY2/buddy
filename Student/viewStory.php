...
$stmt = $conn->prepare("SELECT image, created_at FROM story WHERE student_id = ? AND expires_at > NOW() ORDER BY created_at ASC");
$stmt->bind_param("i", $viewStudentId);
$stmt->execute();
...
// Fetch the next and previous student IDs with active stories
