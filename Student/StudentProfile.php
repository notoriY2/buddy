...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['text_content'])) {
    ...
    $stmt->bind_param("iss", $studentId, $textContent, $image);
    if ($stmt->execute()) {
...
$postId = 1; // Replace with the actual post_id
$stmt = $conn->prepare("
    SELECT 
        s.student_id, 
        s.firstName, 
...
<div class="container">
    ...
    <div class="profile-container" style="margin-top: 60px;">
        <img src="../images/25.png" class="cover-img" alt="Cover Image">
    ...
    ");
    $stmt->bind_param("iii", $studentId, $studentId, $studentId); // Bind the logged-in student's ID
    $stmt->execute();
    $result = $stmt->get_result();
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    button.addEventListener('click', function () {
        ...
        .then(data => {
            if (data.success) {
                // Set the icon color to blue and mark as shared
                this.querySelector('i').style.color = '#F01B2FF';
